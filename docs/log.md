...
- **정식(7,280개) 평가 — dense 단독을 전 지표에서 확실히 넘어섬**:
  ...
  | | dense 단독(ef=200) | off-the-shelf rerank(ef=100) | **파인튜닝 rerank(ef=200, top_n=30)** |
  |---|---|---|---|
  | recall@1 | 0.6768 | 0.6308 | **0.7446** (dense 대비 +6.78%p) |
  | recall@3 | (미측정) | (미측정) | **0.8971** |
  ...
  | latency | 19.47ms | 421.04ms | 442.06ms(p50 441.98, p95 462.17, p99 466.08) |
...
- **결정: 최종 retriever 파이프라인을 "dense 단독"에서 "dense(ef=200) top-30 → 파인튜닝
  ...
  파이프라인에 추가된 것.
- **latency 트레이드오프 — top_n=10으로 축소 테스트 후 top_n=30 유지 쪽으로 판단**: top_n=10
  스모크 테스트(20개 샘플, 정식 평가는 아님) 결과 latency 179ms(top_n=30의 약 40%, 후보
  개수에 거의 비례)까지 줄었지만, 이러면 dense recall@10(0.9386)이 곧 상한이 돼서 top_n=30
  대비 recall 손해가 더 클 걸로 예상됨. 442ms는 UX 응답시간 기준(닐슨: 1초 미만이면 "끊김
  없이 느껴지는" 구간)에도 들어오고, 뒤에 붙을 LLM 생성 시간에 비하면 병목이 아닐 걸로
  판단 — **정확도 우선, top_n=30 유지로 잠정 결론.** latency가 실제 서빙에서 문제가 되면
  그때 top_n 축소/quantization 등 재검토.
- **다음(미실행, 아이디어만 정리됨)**:
  1. **negative 개수 ablation** — 지금 4개 hard negative + in-batch negative(기본 4개)를
     같이 쓰는데, hard negative를 1개로 줄이거나 아예 빼고 in-batch만 쓰면 어떻게 되는지
     비교 실험 제안됨(재생성 없이 `reranker_pairs.jsonl`의 negative_chunks를 잘라서 쓰면
     됨). dense Phase A→B 때 hard negative 추가가 유효했던 것과 같은 논리로, 줄이면
...
## 2026-08-31 — 사건유형별 171건 정식 비교 완료: chunks가 전 지표에서 확실히 우세, OOM 원인도 규명

- **사건유형별 테스트셋 구축**: val_query.json엔 사건유형 필드가 없어서, DB(`case_type`
  컬럼)와 조인해 유형별로 붙인 신규 스크립트 `server/build_typed_test_set.py` 작성.
  전체 22개 사건유형(민사~제조물_책임_민사) 중 유형당 최대 10개씩 샘플링(부족한 유형은
  있는 만큼만) → `data/Val/val_query_by_type.json`, 총 171개(민사/형사 등 13개 유형은
  10개씩, 나머지 9개 유형은 val 자체에 10개가 안 돼서 1~9개). 사건유형 분포 자체가
  극단적으로 편향(민사 18,029건, 형사 12,914건 vs 제조물_책임_민사 9건)돼있음을 확인.
- **`server/run_judge_compare.py` 신설**: `judge_compare.py`의 단일 쿼리 로직을 재사용해
  171개를 순회, 쿼리별 전체 기록(질문/두 답변/judge 판정+reasoning)을 JSONL로 저장하고
  (`server/judge_results.jsonl`, gitignore 처리) 중단 시 자동 이어서 실행(resume) 지원.
  마지막에 전체+사건유형별 집계를 자동 출력.
- **정식 결과 (170/171 완료, 1건은 CUDA OOM — 아래 참고)**:

  | 기준 | full | chunks | tie | 무승부 제외 chunks 승률 |
  |---|---|---|---|---|
  | 정확성 | 49 | 91 | 30 | 65.0% |
  | Groundedness | 41 | 77 | 52 | 65.3% |
  | 인용정확성 | 29 | 78 | 63 | 72.9% |
  | **종합** | 52 | **96** | 22 | **64.9%** |

  종합 기준 무승부 제외 148건 중 96승(64.9%) — 50% 기준 z≈3.6, p<0.001로 **통계적으로
  뚜렷하게 유의미**(사전에 계산해둔 "100건이면 ±10%p 신뢰구간" 기준을 확실히 넘어섬).
  **chunks(매칭 chunk+앞뒤 1개씩, 문서당 3개)가 문서 전체보다 4개 기준 전부 우세.**
- **원인 분석(judge reasoning 직접 확인)**:
  1. **full 버전에서만 완전 실패 사례 발생**(170건 중 2건, chunks는 0건) — `기업_공동행위`
     유형에서 chunks가 9:1로 압승한 이유. judge: "Answer A contains no substantive
     content... fails to answer". `EXAONE-4.0-1.2B`가 1.2B급 소형 모델이라 판례 5개
     원문 전체(수만~수십만 자)를 한 번에 주면 가끔 압도당해 답을 아예 못 만드는 것으로
     추정.
  2. **평균 답변 길이도 chunks가 더 김**(chunks 415자 vs full 359자) — 직관과 반대로
     컨텍스트가 짧은 쪽이 더 충실한 답변을 만듦. 소형 모델이 방대한 컨텍스트에 오히려
     압도된다는 정황과 일치.
  3. **예외: 세무/특허 유형은 full이 우세**(세무 6:4, 특허 4:4 무승부) — 비슷한 판례
     여러 개에 등장하는 숫자·사건번호가 뒤섞이는 오류가 chunks에서 더 잦음("판례들 간
     수치가 뒤섞인 것", "인용문헌을 잘못 표기"). 다수 판례 간 세부 수치 대조가 필요한
     도메인은 문서 전체가 유리할 수 있다는 신호 — 유형별 분기(라우팅) 여지.
- **⚠️ CUDA OOM 원인 규명**: 실패한 쿼리(`기업_공정위`, case_id `2021누48573`)의 top-5
  문서 중 하나(`2017누48`)가 **231,068자**짜리 극단적 이상치 문서였음. `build_context_full`이
  5개 문서를 그대로 이어붙이면서 컨텍스트가 **348,738자**(한글 기준 약 20만 토큰 추정)까지
  불어나 `EXAONE-4.0-1.2B`의 최대 컨텍스트(65,536토큰)조차 넘어설 수 있는 크기가 됨 →
  생성 중 메모리 부족. **같은 쿼리를 chunks 방식으로 만들면 4,954자로 끝남** — 문서
  길이와 무관하게 항상 "매칭 chunk+앞뒤"만큼만 크기가 고정되므로, **full 방식만 가진
  구조적 결함**(코퍼스에 극단적으로 긴 문서 하나만 검색돼도 파이프라인이 죽을 수 있음)이고
  chunks 방식은 이 문제 자체가 원천 차단됨.
- **결정: 컨텍스트 구성 방식을 chunks(매칭 chunk+앞뒤)로 채택.** 정확성/groundedness/
  인용정확성 전부 우세 + 안정성(OOM 회피) + 비용/latency까지 유리 — full을 택할 이유가
  없음. 예외적으로 세무/특허처럼 다수 판례 간 세부 수치 비교가 중요한 유형은 나중에
  full로 분기하는 걸 고려할 수 있음(아직 미구현, 우선순위 낮음).
- **다음**: (1) 사건번호 정규식 라우팅 구현, (2) 답변 불가능 질문 처리(reranker
  cross-encoder 점수를 신뢰도 임계값으로 활용), (3) 필요시 극단적으로 긴 문서 처리(잘라내기
  등) — chunks를 기본 채택했으므로 우선순위는 낮음.

## 2026-08-31 — reranker 점수 threshold로 "관련 판례 없음" 거르기 실험, 결론: 단독으론 불가

- **`CrossEncoderReranker`에 `retrieve_batch_with_scores()` 추가**(`server/eval/retrievers.py`):
  기존 `retrieve_batch`는 순위만 반환하고 cross-encoder 원점수는 버렸는데, "관련 판례
  없음" 판단용 threshold를 실측 근거로 정하려면 점수가 필요해서 1등 후보 점수(top1_score)도
  같이 반환하도록 추가. 기존 `retrieve_batch`는 내부 로직을 `_rerank_one()`으로 뽑아
  공유하도록 리팩터링(동작 동일, 인터페이스 안 바뀜).
- **`server/eval/calibrate_threshold.py` 신설**: val_query.json 전체(7,280개)를 돌려서
  각 쿼리의 top1_score와 recall@5 hit/miss(top-5 안에 정답이 있었는지)를 기록 →
  threshold를 낮은 값부터 올려가며 "hit인데 걸러지는 비율"(놓침, 낮을수록 좋음) vs
  "miss인데 걸러지는 비율"(정직한 거절, 높을수록 좋음)을 스윕. 점수는 sigmoid라 0~1
  범위(실측: 관련 질문 0.66, "오늘 날씨 어때?" 같은 무관 질문 0.23).
- **정식 결과 (7,280개, hit 6,751 / miss 529)**:
  ...
  | threshold | hit 손실(놓침) | miss 포착(거절) | 손실/포착 비율 |
  |---|---|---|---|
  | 0.10 | 0.1%(4개) | 4.2%(22개) | 0.18 |
  | 0.30 | 0.9%(63개) | 11.0%(58개) | 1.09 |
  | 0.50 | 5.2%(354개) | 23.1%(122개) | 2.90 |
  | 0.70 | 16.0%(1,077개) | 41.8%(221개) | 4.87 |
  | 0.90 | 36.6%(2,469개) | 60.7%(321개) | 7.69 |

- **결론: 순수 threshold 단독으로는 불충분.** miss의 중앙값(0.808)이 hit의 25퍼센타일
  (0.806)과 거의 같을 정도로 두 분포가 심하게 겹침 — **"확신은 높은데 틀림"인 miss가
  대부분**이기 때문. 원인: val_query.json의 miss는 "코퍼스에 관련 판례가 아예 없음"이
  아니라 "reranker가 그럴듯한 오답을 확신 있게 골랐음"이라, 확신도 점수로는 이 둘을
  구분할 수 없음. threshold를 올릴수록 miss 포착률보다 hit 손실률이 훨씬 빠르게
  커져서(0.7 기준 miss 1개 정직하게 거절할 때마다 hit 5개 가까이 억울하게 버림) 손해가
  이득보다 큼.
  - **예외**: 아주 낮은 threshold(0.1~0.15)는 가성비가 괜찮음 — hit 손실 0.1~0.2%로
    거의 안 건드리면서 miss의 4~6%는 걸러냄. 이 구간은 "완전히 무관한 질문"(코퍼스에
    진짜 관련 내용이 없는 경우)일 가능성이 높음 — 다만 val_query.json엔 이런 진짜
    무관 질문이 없어서 직접 확인은 못 함.
- **다음 방향(하이브리드, 이전 논의와 일치)**: 아주 낮은 threshold로 "명백히 무관한
  질문"만 저비용으로 걸러내고(1차 필터), 그 이상 애매한 영역은 threshold로 해결 안
  되므로 LLM이 실제 판례 내용을 읽고 최종 판단하게 위임(2차, generate.py 프롬프트에
  이미 있는 "답할 수 없으면 솔직히 말하라" 지시 활용). threshold 값 자체(예: 0.1~0.15
  중 어디)와 LLM 판단 위임 로직은 아직 미구현.

## 2026-09-01 — threshold=0.2 확정 + LLM 자체 거절 실측 + GPT judge로 "라벨 대신 진짜 관련성" 검증, 답변불가 기능 자체의 실효성에 의문 제기

- **threshold=0.2로 확정**: 앞서 스윕한 threshold별 "hit 손실 ÷ miss 포착" 누적 비율을
  계산해보면 0.30에서 비율이 1.09로 역전(손실>포착)됨 — 그 이전인 **0.2(비율 0.69)**가
  hit 손실은 절대치로 작으면서(0.4%, 27/6,751) miss도 의미 있게(7.4%, 39/529) 잡는
  지점으로 채택.
- **`CrossEncoderReranker.retrieve_batch_with_scores()` 신설** (`server/eval/retrievers.py`):
  기존 SQL+재점수 로직을 `_rerank_one()`으로 공통화하고, case_no 랭킹뿐 아니라 1등
  cross-encoder 점수(top1_score)도 반환하도록 확장(기존 `retrieve_batch`는 그대로 유지,
  하위호환).
- **`server/check_miss_rejection.py` 신설**: threshold=0.2로 안 걸러지는 miss 490개에
  대해 실제 답변 생성 모델(EXAONE)이 "관련 판례 없음"을 스스로 인지하는지 확인. 처음엔
  GPT judge로 판정하려 했으나 **OpenAI API 크레딧 소진**(`insufficient_quota`)으로 실패 —
  API 의존 없이 키워드 패턴(정규식) 기반 거절 탐지로 전환(원본 답변은 다 저장해둬서
  나중에 GPT로 재검증 가능). **결과: 490개 중 거절 10개(2.0%)뿐** — threshold를 통과한
  애매한 영역에서는 LLM 스스로도 거의 못 알아챔.
- **핵심 문제 제기(사용자)**: 지금까지 "miss"는 val_query.json의 **라벨(case_id) 기준**일
  뿐, "top-5에 진짜 쓸만한 판례가 없다"는 뜻이 아닐 수 있음 — 법률 질문은 관련 판례가
  여러 개일 수 있어 라벨과 다른 문서로도 답변 가능했을 가능성. **hit 쪽도 확인 필요**
  (threshold를 통과한 hit에서 LLM이 괜히 거절하는 비율은 그동안 한 번도 측정 안 함).
- **`server/eval_answer_quality.py` 신설**: hit 100개(새로 답변 생성) + miss 100개
  (거절 10개 전부 + 미거절 90개, `check_miss_rejection.py` 결과 재사용)를 GPT-5-mini로
  평가. 라벨이 아니라 **실제 top-5 문서 내용을 보고 GPT가 새로 판단한 진짜 관련성**
  (`actually_relevant`)을 기준으로, 시스템의 실제 행동(답변/거절)이 옳았는지 확인하는
  통일 스키마 사용. 병합 형사사건(사건번호에 쉼표+"(병합)" 포함, 예:
  `2021고단3537, 2141(병합)`)은 파일명과 안 맞아 5건 스킵(치명적이지 않음, 총 195/200 완료).
- **정식 결과**:

  | 카테고리 | n | 지표 | 결과 |
  |---|---|---|---|
  | hit | 100 | answer_correct | 87/100 (87.0%) |
  | miss-거절 | 10 | decline_justified | **1/10 (10.0%)** |
  | miss-미거절 | 85 | actually_relevant | **85/85 (100.0%)** |
  | miss-미거절(관련 있던 것 중) | 85 | answer_correct | 66/85 (77.6%) |

- **결론 — 예상을 뒤집는 발견 두 가지**:
  1. **미거절 miss 85개 전부 실제로는 관련 문서가 있었음(100%)** — 라벨상 "오답"의
     대부분이 사실은 "다른 관련 판례로 답변 가능했던 경우". recall@5(92.73%)가 실제
     유용성을 상당히 **과소평가**하고 있었다는 뜻 — 라벨과 정확히 일치하는 문서가 아니어도
     실질적 답변 가능성은 훨씬 높음.
  2. **거절한 10개 중 9개(90%)는 거절하면 안 됐음** — EXAONE의 자체 거절 판단이 대부분
     틀림. threshold도 못 거르고 LLM 자기 판단도 못 미더움이 실측으로 확인됨.
  - **종합: "답변 불가능 처리" 기능 자체의 실효성에 의문.** 이 도메인(실제 법률 질문
    분포)에서는 top-5에 진짜 관련 문서가 아예 없는 경우가 매우 드묾 — val_query.json
    같은 도메인-내 질문에 한해서는, 무관한 threshold(0.2)로 명백히 엉뚱한 질문(예: "오늘
    날씨 어때?")만 걸러내고 그 이상은 **거절 로직 없이 항상 답변을 시도하는 편**이 오히려
    나을 수 있음(거절 기능이 실제로는 손해를 더 만듦). 다만 이 결론은 도메인-내 질문
    표본 기준이라, 진짜 완전히 무관한(도메인 밖) 질문에 대한 threshold의 효과는 여전히
    유효(이전 실험, hit 0.1~0.2% 대비 miss 4~6% 저비용 필터).
