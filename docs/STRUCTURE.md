...
결과는 여기 말고 log.md에 있음(로컬 전용, 공개 저장소엔 미포함) — 이 문서는 "무엇이
어디 있는지"만 다룸.
...
| 파일 | 용도 |
|---|---|
| `PROJECT_STATE.md` | *(로컬 전용, 공개 저장소엔 미포함)* **새 세션 시작 시 제일 먼저 읽는 파일.** 현재 상태 요약 + 다음 계획 |
| `DECISIONS.md` | 전체 프로젝트 흐름 요약 — 각 단계 후보/결정/이유만 (세부 수치 제외, log.md가 너무 길어서 큰 그림만 보려고 만든 문서) |
| `log.md` | *(로컬 전용, 공개 저장소엔 미포함)* 날짜순 작업 로그 — 모든 실험 결과, 원인 분석, 결정 근거의 원본 기록 |
