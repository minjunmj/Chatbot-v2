...
- RAG 답변 품질 평가
  ...
  - [x] 답변 불가능한 질문 처리 — **조사 완료, 기능 자체의 실효성에 의문 제기로 결론.**
    ...
    top-5 실제 내용 기준 진짜 관련성 판정)로 정식 검증한 결과: **미거절 miss 85개 중
    85개(100%) 실제로 관련 문서 있었음**(recall@5가 실제 유용성을 과소평가), **거절한
    10개 중 9개(90%)는 거절하면 안 됐음**(LLM 자체 판단 신뢰 안 됨). 이 도메인(실제
    법률 질문)에서는 top-5에 진짜 관련 문서가 없는 경우가 드물어서, **명백히 도메인
    밖인 질문만 threshold=0.2로 거르고 그 이상은 거절 로직 없이 항상 답변 시도하는 편이
    나을 수 있음**으로 결론. LLM 자체 거절 위임 로직은 신뢰도 부족 확인돼 보류 —
    [log.md](log.md) 2026-09-01
