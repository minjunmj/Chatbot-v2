...
이 프로젝트는 **LLM Application Engineer 포트폴리오**로 진행했습니다. "일단 되게
만드는 것"이 아니라, 임베딩 모델·chunking 전략·retriever 구성·컨텍스트 구성 방식·
프롬프트 등 각 선택지마다 후보를 비교하고 정량적 근거로 결정하는 과정 자체를
프로젝트의 핵심으로 삼았습니다. 그 과정에서 나온 실패(하이브리드 검색 미채택,
프롬프트 개선 시도 실패 등)도 숨기지 않고 그대로 기록했습니다.
